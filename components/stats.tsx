...
export function Stats() {
  ...
  // Real-world dynamic data state, defaulting to fallbacks
  const [githubRepos, setGithubRepos] = useState(48)
  const [leetcodeSolved, setLeetcodeSolved] = useState(420)
  const [hackerRankBadges, setHackerRankBadges] = useState(12)
  const [codeforcesContests, setCodeforcesContests] = useState(35)

  useEffect(() => {
    // Generate deterministic heatmap based on current date so it doesn't flash randomly on reload
    const seed = new Date().getDay();
    setHeat(Array.from({ length: WEEKS * DAYS }, (_, i) => {
      // Create some cluster patterns resembling a real commit history
      const val = Math.floor(Math.abs(Math.sin(i * 0.5 + seed)) * 5)
      // Randomly turn some off to make it look organic
      return Math.random() > 0.3 ? val : 0
    }))
  ...
  const STATS = [
    { icon: GitBranch, label: "GitHub", metric: "Repositories", value: githubRepos, suffix: "+" },
    { icon: Code2, label: "LeetCode", metric: "Problems Solved", value: leetcodeSolved, suffix: "+" },
    { icon: Activity, label: "HackerRank", metric: "Badges", value: hackerRankBadges, suffix: "" },
    { icon: Trophy, label: "Codeforces", metric: "Contests", value: codeforcesContests, suffix: "+" },
  ]
